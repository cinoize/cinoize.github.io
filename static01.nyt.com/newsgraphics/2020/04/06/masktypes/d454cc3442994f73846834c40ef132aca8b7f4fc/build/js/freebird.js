import { i as isMobile } from './chunks/page.2b523c01.js';
...
const defaults = {
	...
	// must be in localhost.nytimes.com to see this.
	env: 'production',
	dataEnv: 'production',
...
// https://github.com/nytm/vhs3/blob/master/doc/OPTIONS.md
function getVHSOptions(d) {
	d.asset = "" + d.asset; // scoop strings can become numbers

	const src = isMobile() ? d.mobileAsset || d.asset : d.asset;
	const ratio = getRatio(d.vhsOptions.ratio);

	const vid = d.asset.split("/")[d.asset.split("/").length - 1].split(".")[0];

	switch(d.type) {
		case 'audio':
			// https://github.com/nytm/vhs3/blob/master/doc/AUDIO.md
			return assign({}, {
				nytd: 'audiourl-' + vid,
				id: 'audiourl-' + vid,
				mediaType: 'audio',
				api: false,
				src: src,
				headline: d.headline || "Audio",

				width: '100%',

				audioControls: { // optional
					fastForward: false, // fast forward button. off by default.
					rewind: false, // rewind button. off by default.
					volume: true, // volume button. on by default for desktop.
					// permanently off (regardless of this parameter) for touch devices.
					suppressDurationPlaceholder: true, // This prevents "Play Audio" from being displayed,
					// which occurs before playback if the duration is not available.
					// off by default.
					// durationStyle: { fontSize: '20px' }, // `undefined` by default. a way to override the text styles
					// applied to the duration text and its placeholder.
					// If you change the font in `durationStyle`:
					// *Please* use a font with monospace numbers.
					// Otherwise, the player will jitter when counting up or down.
				}
			}, d.vhsOptions);

		case 'scoop-video':
			return assign({}, defaults, {
				nytd: 'scoop-video-' + d.asset,
				id: ""+src,
				api: true,
				ads: true
			}, d.vhsOptions);
		...
		case 'videotape':
			// responsive
			// TODO multiple image posters?
			let sources = [];
			let posterUrl = '';
			if (d.sizes) {
				const sizes = d.sizes.split(/[,\s]+/).filter(Boolean);
				const base = src.substring(0,src.lastIndexOf("-") + 1);
				sizes.forEach(size => {
					size = +size;
					sources.push({
						width: size,
						height: Math.round(size*ratio[1]/ratio[0]),
						url: base + size +  "w.mp4"
					});
				});
			}
				...
				// 	"video/mp4": src
				// }
				// posterUrl = src.replace(/\.[a-zA-Z0-9]+$/, '.jpg');
				const w = src.substring(src.lastIndexOf("-") + 1).replace(/\D/g,'') || 1280;
				sources.push({
					width: w,
					height: Math.round(w*ratio[0]/ratio[1]),
					url: src
				});
			}

			// todo be smarter about this.
			posterUrl = sources[sources.length-1].url.replace(/\.[a-zA-Z0-9]+$/, '.jpg');
			//posterUrl: '', // img or mp4
		...
		default:
			console.log("Video Error: Unknown video type " + d.type);
			console.log(d);
	}
}
...
function get_data(element) {
	...
	for (const k in element.dataset) {
		obj[k] = try_parse(element.dataset[k]);
	}
	return obj;
}
...
function isMobileRatio(d) {
	return isMobile() && d.mobileAsset !== undefined && d.mobileRatio !== undefined;
}

function setVHSRatio(d) {
	const data = d;
	data.vhsOptions.ratio = d.mobileRatio;
	return data;
}

function setPadding(d, el) {
	const ratio = getRatio(d.vhsOptions.ratio);
	const ratioDecimal = (ratio[1]/ratio[0]) * 100;
	el.style.paddingBottom = `${ratioDecimal}%`;
}

function getRatio(ratio) {
	return ratio ? ratio.split(':') : [16,9];
}

const targets = [].slice.call(document.querySelectorAll('.g-vhs'));

const players = typeof VHS === 'undefined' ? [] : targets.map((el, count) => {
	let data = get_data(el);
	if (isMobileRatio(data)) {
		data = setVHSRatio(data);
		setPadding(data, el);
	}
	const options = getVHSOptions(data);

	options.container = el.id;

	// do we want manual control of this?
	if (data.allowMultiplePlayers) {
	...
	if (data.type === 'audio') {
		player = VHS.audio(options);
	}
	if (data.cinemagraph) {
		player = VHS.cinemagraph(options);
	}
	else {
		player = VHS.video(options);
	}
