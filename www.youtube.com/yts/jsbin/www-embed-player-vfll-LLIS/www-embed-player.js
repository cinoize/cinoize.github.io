...
function c(t,w){if("string"===typeof t){t=unescape(encodeURIComponent(t));for(var p=[],H=0,V=t.length;H<V;++H)p.push(t.charCodeAt(H));t=p}w||(w=t.length);p=0;if(0==k)for(;p+64<w;)b(t.slice(p,p+64)),p+=64,q+=64;for(;p<w;)if(f[k++]=t[p++],q++,64==k)for(k=0,b(f);p+64<w;)b(t.slice(p,p+64)),p+=64,q+=64}
function d(){var t=[],w=8*q;56>k?c(g,56-k):c(g,64-(k-56));for(var p=63;56<=p;p--)f[p]=w&255,w>>>=8;b(f);for(p=w=0;5>p;p++)for(var H=24;0<=H;H-=8)t[w++]=e[p]>>H&255;return t}
...
parseInt(g,10);isFinite(g)&&0<g&&(d="ST-"+cb(d).toString(36),b=b?Td(b):"",pg(d,b,g||5))}else g="ST-"+cb(d).toString(36),b=b?Td(b):"",pg(g,b,5)}}}if(c)return!1;if((window.ytspf||{}).enabled)spf.navigate(a);else{var l=void 0===l?{}:l;var k=void 0===k?"":k;var q=void 0===q?window:q;c=q.location;a=Ud(a,l)+k;a=a instanceof G?a:Tb(a);c.href=Rb(a)}return!0}
...
;function si(a){if("1"!==hb(be(),"args","privembed")){a&&ce();try{ri().then(function(b){R("pass_biscotti_id_in_header_ajax")||(b=ie(b),b.bsq=ti++,He("//www.youtube.com/ad_data_204",{va:!1,C:b,withCredentials:!0}))},function(){}),S(si,18E5)}catch(b){Q(b)}}}
