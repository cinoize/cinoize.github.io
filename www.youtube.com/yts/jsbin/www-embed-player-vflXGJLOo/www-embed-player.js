...
b.prototype.then=function(h,g){function k(v,p){return"function"==typeof v?function(C){try{l(v(C))}catch(K){n(K)}}:p}
...
b.all=function(h){var g=q(h),k=g.next();return k.done?d([]):new b(function(l,n){function r(C){return function(K){v[C]=K;p--;0==p&&l(v)}}
var v=[],p=0;do v.push(void 0),p++,d(k.value).X(r(v.length-1),n),k=g.next();while(!k.done)})};
return b});
...
function d(g,k){var l=k&&typeof k;"object"==l||"function"==l?f.has(k)?l=f.get(k):(l=""+ ++h,f.set(k,l)):l="p_"+k;var n=g.g[l];if(n&&sa(g.g,l))for(var r=0;r<n.length;r++){var v=n[r];if(k!==k&&v.key!==v.key||k===v.key)return{id:l,list:n,index:r,u:v}}return{id:l,list:n,index:-1,u:void 0}}
...
function d(){var r=[],v=8*n;56>l?c(g,56-l):c(g,64-(l-56));for(var p=63;56<=p;p--)f[p]=v&255,v>>>=8;b(f);for(p=v=0;5>p;p++)for(var C=24;0<=C;C-=8)r[v++]=e[p]>>C&255;return r}
for(var e=[],f=[],h=[],g=[128],k=1;64>k;++k)g[k]=0;var l,n;a();return{reset:a,update:c,digest:d,ta:function(){for(var r=d(),v="",p=0;p<r.length;p++)v+="0123456789ABCDEF".charAt(Math.floor(r[p]/16))+"0123456789ABCDEF".charAt(r[p]%16);return v}}}
...
var Fc;
function Gc(){var a=u.MessageChannel;"undefined"===typeof a&&"undefined"!==typeof window&&window.postMessage&&window.addEventListener&&-1==Ab.indexOf("Presto")&&(a=function(){var e=document.createElement("IFRAME");e.style.display="none";Kb(e);document.documentElement.appendChild(e);var f=e.contentWindow;e=f.document;e.open();e.write(Fb(Hb));e.close();var h="callImmediate"+Math.random(),g="file:"==f.location.protocol?"*":f.location.protocol+"//"+f.location.host;e=A(function(k){if(("*"==g||k.origin==g)&&
...
var O=window.ytcsi&&window.ytcsi.now?window.ytcsi.now:window.performance&&window.performance.timing&&window.performance.now&&window.performance.timing.navigationStart?function(){return window.performance.timing.navigationStart+window.performance.now()}:function(){return(new Date).getTime()};var Zd=window.yt&&window.yt.config_||window.ytcfg&&window.ytcfg.data_||{};y("yt.config_",Zd,void 0);function P(a){Yd(Zd,arguments)}
...
function ig(){S("web_gel_timeout_cap")&&!dg&&(dg=T(gg,6E4));U(cg);cg=T(gg,Q("LOGGING_BATCH_TIMEOUT",oe("web_gel_debounce_ms",1E4)))}
...
function vg(a){a=Object.assign({},a);delete a.Authorization;var b=Bc();if(b){var c=new Rc;c.update(Q("INNERTUBE_API_KEY",void 0));c.update(b);b=c.digest();c=!0;Ba(b);!1===c||void 0===c?c=0:!0===c&&(c=3);if(!Tb){Tb={};for(var d="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".split(""),e=["+/=","+/","-_=","-_.","-_"],f=0;5>f;f++){var h=d.concat(e[f].split(""));Sb[f]=h;for(var g=0;g<h.length;g++){var k=h[g];void 0===Tb[k]&&(Tb[k]=g)}}}c=Sb[c];d=[];for(e=0;e<b.length;e+=3){var l=b[e],
n=(f=e+1<b.length)?b[e+1]:0;k=(h=e+2<b.length)?b[e+2]:0;g=l>>2;l=(l&3)<<4|n>>4;n=(n&15)<<2|k>>6;k&=63;h||(k=64,f||(n=64));d.push(c[g],c[l],c[n]||"",c[k]||"")}a.hash=d.join("")}return a}
...
timeout:d.timeout,withCredentials:!0},f="",h=a.f.Ha;h&&(f=h);h=a.f.Ia||!1;var g=ug(h,f,d);Object.assign(e.headers,g);e.headers.Authorization&&!f&&(e.headers["x-origin"]=window.location.origin);var k=ie(""+f+("/youtubei/"+a.f.Ba+"/"+b),{alt:"json",key:a.f.Aa}),l;if(d.retry&&S("retry_web_logging_batches")&&"www.youtube-nocookie.com"!=f&&(l=zg(b,c,g,h))){var n=e.onSuccess,r=e.ja;e.onSuccess=function(v,p){Ag(l);n(v,p)};
c.ja=function(v,p){Ag(l);r(v,p)}}try{S("use_fetch_for_op_xhr")?ye(k,e):He(k,e)}catch(v){if("InvalidAccessError"==v)l&&(Ag(l),l=0),R(Error("An extension is blocking network request."),"WARNING");
...
function eh(a,b){var c="ytPlayer"+b+a.D;a.i[b]=c;u[c]=function(d){var e=T(function(){if(!a.g){a.G.L(b,d);var f=a.R,h=String(e);h in f&&delete f[h]}},0);
Za(a.R,String(e))};
return c}
...
m.m=function(){ah(this);if(this.j&&this.f&&this.j.destroy)try{this.j.destroy()}catch(b){R(b)}this.V=null;for(var a in this.i)u[this.i[a]]=null;this.Z=this.f=this.api=null;delete this.U;delete this.l;L.prototype.m.call(this)};
