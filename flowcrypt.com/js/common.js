/* Business Source License 1.0 © 2016-2019 FlowCrypt Limited (human@flowcrypt.com). Use limitations apply. This version will change to GPLv3 on 2020-01-01. See https://github.com/FlowCrypt/cryptup-browser/tree/master/src/LICENCE */
...
(function ( /* ALL TOOLS */) {
  ...
  var tool = window.tool = {
    ...
    api: {
      ...
      cryptup: {
        ...
        help_waitlist: api_cryptup_help_waitlist,
        account_login: api_cryptup_account_login,
        account_login_web: api_cryptup_account_login_web,
        account_check: api_cryptup_account_check,
        billing_pricing: api_cryptup_billing_pricing,
        billing_overview: api_cryptup_billing_overview,
        billing_payments: api_cryptup_billing_payments,
        billing_seats_list: api_cryptup_billing_seats_list,
        billing_seats_assign: api_cryptup_billing_seats_assign,
        billing_seats_purchase: api_cryptup_billing_seats_purchase,
        billing_seats_purchase_additional: api_cryptup_billing_seats_purchase_additional,
  ...
  function api_auth_process_and_save_fragment(fragment, renewing_tokens_on_account_email, state_must_match_this_frame_id, callback) {
    ...
    if (!tool.value(api_outlook_oauth_config.state_header).in(token_response.state)) {
      callback(false, 'FlowCrypt state not present in this result');
    } else {
      ...
      if (Date.now() < storage.microsoft_auth.expires_on) {
        ...
        silent_implicit_token_refresh_in_hidden_iframe(account_email, function (auth_success, auth_result) {
          if (auth_success) {
            ...
            alert('Outlook needs you to sign in again to continue using FlowCrypt.');
  ...
  function api_attester_call(path, values, callback, format) {
    api_call(api_cryptup_url('attester'), path, values, callback, format || 'JSON');
  }
  ...
  function api_cryptup_url(type, variable) {
    let prod = {
      ...
      'pubkey': 'https://flowcrypt.com/me/' + variable + '/pub',
    ...
    let dev = {
      ...
      'me': 'http://127.0.0.1:5001/me/' + variable,
      'pubkey': 'http://127.0.0.1:5001/me/' + variable + '/pub',
      'decrypt': 'http://127.0.0.1:5001/' + variable,
      'web': 'http://127.0.0.1:5001/',
      'attester': 'http://127.0.0.1:5002/',
    };
    return (window.location.host === 'l.flowcrypt.com' ? dev : prod)[type];
  }
  ...
  function api_cryptup_help_waitlist(email, waitlist, callback) {
    return api_cryptup_call('help/waitlist', {
      email: email,
      waitlist: waitlist,
    }, api_cryptup_response_formatter(callback));
  }
  ...
  function api_cryptup_billing_pricing(seats, seats_add, callback) {
    api_cryptup_call('billing/pricing', {
      seats: seats,
      seats_add: seats_add || 0,
    }, api_cryptup_response_formatter(callback));
  }

  function api_cryptup_billing_seats_list(callback) {
    api_cryptup_call('billing/seats_list', {
      account: localStorage.flowcrypt_account_email, // todo - better/more general solution to pass creds
      uuid: localStorage.flowcrypt_account_uuid,
    }, api_cryptup_response_formatter(callback));
  }

  function api_cryptup_billing_seats_assign(user_emails_add, callback) {
    api_cryptup_call('billing/seats_assign', {
      account: localStorage.flowcrypt_account_email, // todo - better/more general solution to pass creds
      uuid: localStorage.flowcrypt_account_uuid,
      user_emails_add: user_emails_add,
    }, api_cryptup_response_formatter(callback));
  }

  function api_cryptup_billing_overview(callback) {
    api_cryptup_call('billing/overview', {
      account: localStorage.flowcrypt_account_email, // todo - better/more general solution to pass creds
      uuid: localStorage.flowcrypt_account_uuid,
    }, api_cryptup_response_formatter(callback));
  }

  function api_cryptup_billing_payments(callback) {
    api_cryptup_call('billing/payments', {
      account: localStorage.flowcrypt_account_email, // todo - better/more general solution to pass creds
      uuid: localStorage.flowcrypt_account_uuid,
    }, api_cryptup_response_formatter(callback));
  }

  function api_cryptup_billing_seats_purchase(product, method, source, seats, callback) {
    api_cryptup_call('billing/seats_purchase', {
      account: localStorage.flowcrypt_account_email, // todo - better/more general solution to pass creds
      uuid: localStorage.flowcrypt_account_uuid,
      product: product,
      method: method,
  ...
  function api_cryptup_billing_seats_purchase_additional(seats_add, callback) {
    api_cryptup_call('billing/seats_purchase', {
      account: localStorage.flowcrypt_account_email, // todo - better/more general solution to pass creds
      uuid: localStorage.flowcrypt_account_uuid,
      product: null,
  ...
  function api_cryptup_account_login_web(redirect, callback) {
    ...
    tool.each(items, function (i, item) {
      ...
      api_call(item.base_url, '', values, function (success, s3_result) {
        ...
        if (results.reduce(function (sum, r) { return sum + (typeof r !== 'undefined') }, 0) === results.length) { // no undefined left
          ...
          upload: function (single_file_progress) {
